...
* **Detecção e Voz Inteligente NF-e (Novo):** Remoção total da necessidade de chaves/switches manuais para bipagem. O leitor identifica dinamicamente se o código bipado possui exatamente 44 caracteres numéricos (uma chave de Nota Fiscal Eletrônica). Caso uma NF-e seja bipada logo após um item comum, o sistema aciona a síntese de voz nativa do navegador anunciando *"Nota fiscal detectada!"*, retornando ao som de bipe convencional nos itens seguintes, garantindo total ergonomia de mãos livres para o operador.

...
* **Detecção e Voz Inteligente NF-e:** Remoção total da necessidade de chaves/switches manuais para bipagem. O leitor identifica dinamicamente se o código bipado possui exatamente 44 caracteres numéricos (uma chave de Nota Fiscal Eletrônica). Caso uma NF-e seja bipada logo após um item comum, o sistema aciona a síntese de voz nativa do navegador anunciando *"Nota fiscal detectada!"*, retornando ao som de bipe convencional nos itens seguintes, garantindo total ergonomia de mãos livres para o operador.
* **Extração Avançada de Chaves:** Extração inteligente e automática da chave de 44 dígitos mesmo se o operador bipar o QR Code completo da nota ou URLs contendo a chave, facilitando a captura sem necessidade de formatação manual.
* **Calendário de Produção Inteligente:** Design moderno totalmente integrado com codificação de cores automática de acordo com as categorias ativas em cada dia. Permite clicar em qualquer dia para abrir um modal de detalhes dinâmico, contendo sanfona de lotes, filtros inteligentes de categoria e barra de busca instantânea de códigos ou lotes.
...
* **Operador Dinâmico em Documentos (Novo):** As placas de contêineres, manifestos de carga e chancelas de autenticação agora exibem dinamicamente o **Nome do Operador Ativo** que está logado no sistema, eliminando completamente as assinaturas estáticas e assegurando integridade e rastreabilidade total de responsabilidade técnica por lote.
...
Consulte o arquivo [CHANGELOG.md](CHANGELOG.md) para o histórico completo de atualizações e modificações do sistema.
